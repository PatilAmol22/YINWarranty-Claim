...
import getCurrentYearQuaterledger from '@salesforce/apex/YINOrderManagementController.getCurrentYearQuaterledger';
import promotionalOrderSendForApproval from '@salesforce/apex/YINOrderManagementController.promotionalOrderSendForApproval';
import { loadScript, loadStyle } from 'lightning/platformResourceLoader';
import customcss from '@salesforce/resourceUrl/resourceOrderMangmt';
import make_payment_url from '@salesforce/label/c.make_payment_url'
import LightningAlert from 'lightning/alert';
import { NavigationMixin } from 'lightning/navigation';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import LightningConfirm from 'lightning/confirm';
//import InStockText from '@salesforce/resourceUrl/InStockText';
//import LimitedStockText from '@salesforce/resourceUrl/LimitedStockText';
//import OutOfStock from '@salesforce/resourceUrl/OutOfStock';
//import UpwardArrow from '@salesforce/resourceUrl/UpwardArrow';
//import DownWardArrow from '@salesforce/resourceUrl/DownWardArrow';
 ...
 */
export default class YinOrderManagementCmp extends NavigationMixin(LightningElement){
    //InStockText=InStockText;
    //LimitedStockText=LimitedStockText;
   ...
   */
    ...
    grandTotal = 0;
    totalEPRAmount=0;
    canNotBeProcess = false;
    isGridView = true;
    defaultShipAdd=false;//added by kalpesh chande -8-07-2024
    @track isCreatedFromMobile=false;   
    selectedOrderType ={
        all:true,
        discount:false
    }
    ...
    // geolander = 'https://yokohama-india--dev--c.sandbox.vf.force.com/resource/Earth_1';
    ...
    get isPromotional(){
        return this.orderModuleType=='Promotional';
    }
//Commented by Kalpesh Chande -21-08-2024 -On double click ERP Service called twice
    // get isDisableBtn(){
    //     return this.isLoading;
    // }
    @track isDisableBtn=false;//Added By Kalpesh Chande -21-08-2024
    ...
    async connectedCallback(){
        ...
        if(!this.orderModuleType){
            if(url.searchParams.get('c__module')){
                this.orderModuleType = url.searchParams.get('c__module');
            }
        }
        ...
        if(this.isPromotional){
            this.bugetInfo = await getCurrentYearQuaterledger({accountId:this.accountId,recordType:'Promotional_Budget'});
            let totalBudget = Number(this.bugetInfo.totalBudget).toFixed(2);
            let availableBudget = Number(this.bugetInfo.availableBudget).toFixed(2);
            console.log('this.bugetInfo ',this.bugetInfo);
            this.bugetInfo = {
      ...
      //  console.log('Cart Detail image link ',this.cartDetails.productImage);
        ...
        this.shippingAddressOption = addresses;
         //added by kalpesh chande -8-07-2024 start here
         if(this.shippingAddressOption.length==2){
             this.defaultShipAdd=true;
             this.shippingAddress =  this.billingAddress;
             this.shippingAccountValue=this.shippingAddressOption[1].value;
            }
         //added by kalpesh chande -8-07-2024 end here
    ...
    cartCalculation(){
      ...
      //  let totalEPRAmount=0;
        ...
        this.cartDetails.forEach(item=>{
           ...
           // let eprAmount = isNaN(item.pricebookEntry?.Product2.EPR_Charges__c * item.quantity)?0:item.pricebookEntry?.Product2.EPR_Charges__c * item.quantity;
            let discount = (listPrice * item.discountPercentage)/100;
           // console.log('item.eprAmount>>>>1',item.eprAmount);
           // item.eprAmount=eprAmount;//Added By Kalpesh Chande -25-07-2024 
           // console.log('item.eprAmount>>>>2',item.eprAmount);
            item.netPrice = listPrice;
            item.eprRate=(item.pricebookEntry?.Product2.EPR_Charges__c != null)?item.pricebookEntry?.Product2.EPR_Charges__c:0;
           ...
           // totalEPRAmount=totalEPRAmount+eprAmount;
      ...
      //  this.totalEPRAmount=isNaN(totalEPRAmount)?0:Number(totalEPRAmount).toFixed(2);
    ...
    async handleChangeToggle(event){
        loadProductEPR();//Added By Kalpesh Chande -28-06-2024
    ...
    async handleCartAdd(event){
        ...
        //Added By Kalpesh Chande -28-06-2024 -EPR  Amount -Start
        if(productWrap.eprRate <=0 && this.isPromotional==false){
            this.showAlert(`Selected product have an invalid PRC rate: ${productWrap.pricebookEntry.Product2.Product_Code__c}`, 'error', 'Error');
            loadProductEPR();
            return;
        }
        //Added By Kalpesh Chande -28-06-2024 -EPR  Amount -End
        let cartIndex = undefined;
        if(this.selectedOrderType.all){ // Form Normal Product , No Need to check Variants
            cartIndex = this.cartDetails.findIndex(ele=>ele.productId==productWrap.productId && !ele.variantId);
        ...
        if(cartIndex==-1){
            ...
            let productWrap = this.productswrapper[index];
            if(this.accountDetails.Is_Locking_Enable__c && this.isPromotional==false){
            ...
            if(this.accountDetails.Is_Capping_Enable__c && this.isPromotional==false){
          ...
          //  productWrap.eprRate=(productWrap.pricebookEntry?.Product2.EPR_Charges__c != null)?productWrap.pricebookEntry?.Product2.EPR_Charges__c:0;
            ...
            //  Refresh Carts
            this.cartDetails = await getCartDetails({accountId:this.accountId,openOrderId:this.openOrderId,orderModuleType:this.orderModuleType});
            console.log('cart epr rate>>', this.cartDetails )
            this.CartDetailLength = this.cartDetails.length;
            this.cartCalculation();
    ...
    async handleCartDelete(event){
        ...
        if(cartIndex!=-1){
            ...
            productWrap.isPromotional = this.isPromotional;
    ...
    async handleEnter(event){
        console.log('handle Enter ',event.key);
        if(event.key==='Enter'){
    ...
    async handleCheckout(event){
        ...
        //Added By Kalpesh Chande -21-08-2024 -Start here 
        let buttonText = event.target.innerText;
        console.log('Button text: ', buttonText);
        if (buttonText === 'Place Order' && this.isDisableBtn) {
        ...
        //Added By Kalpesh Chande -21-08-2024 -End here
        if(!this.isPromotional){
            ...
            if(this.shippingAddress && this.shippingAccountValue){
                try {
                    //Added By Kalpesh Chande -28-06-2024 - EPR Amount -Start Here
                    let invalidProducts = this.cartDetails.filter(product => product.eprRate == null || product.eprRate == 0);
                    if (invalidProducts.length > 0 && this.isPromotional==false) {
                    let productNames = invalidProducts.map(product => product.pricebookEntry.Product2.Product_Code__c).join(', ');
                    console.log('After Error epr productNames '+productNames);
                    this.showAlert(`The following products have an invalid PRC rate: ${productNames}`, 'error', 'Error');
                    this.isLoading = false;
                    return;
                    }
                    console.log('After Error epr');
                     //Added By Kalpesh Chande -28-06-2024 - EPR Amount-End Here
                   //  this.isCreatedFromMobile=false;
                    ...
                    console.log('error ',error);
                    if(!error.body.message.includes('Credit Days Expired') && !error.body.message.includes('Credit Limit Exceed')){
                        this.showAlert(error.body.message,'error','Error');
                    }else{
            ...
            try {
                if(this.shippingAddress && this.shippingAccountValue){
                if(this.bugetInfo.availableBudget < this.grandTotal){
                    this.showAlert('Promotional Available Budget Exceed','error','Error');
                    this.isLoading = false;
                    return;
                }
    ...
    async changeCartQuantity(event){
        ...
        item.quantity = isNaN(quantity)?0:Number(parseInt(quantity));
    ...
    changeQuantity(event){
        ...
        if(type=='change'){
            this.productswrapper[index].quantity = isNaN(value)?0:Number(parseInt(value));
        }
        this.productswrapper[index] = JSON.parse(JSON.stringify(this.productswrapper[index]));
    ...
    async showPrompt(message,theme,label){
        ...
        if(result){// When click OK
            console.log('Ok Clicked');
            try {
                this.handleMakePaymentNavigation();
            } catch (error) {
                console.log('error ',error);
                console.log('error ',error.message);
            }
...
//Added by Kalpesh Chande -28-06-2024
    async loadProductEPR(){
        let trendingSKU = this.refs.trendingSKU.checked;
        let productOfTheMonth = this.refs.productOfTheMonth.checked;
    ...
    handleMakePaymentNavigation() {
        location.replace(this.customlabels.make_payment_url);
    }
}